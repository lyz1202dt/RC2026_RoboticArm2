...
ArmTaskNode::ArmTaskNode(const rclcpp::NodeOptions& options)
    ...
    place_target_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
        kPlaceTargetTopic, 10, std::bind(&ArmTaskNode::on_place_target_pose, this, std::placeholders::_1));
...
void ArmTaskNode::execute_move_to_position(int preset_position_id) {
    ...
    if (iter == arm_positions_.end()) {
        RCLCPP_ERROR(get_logger(), "Preset position %d not found in arm_position.yaml", preset_position_id);
        return;
    }
...
void ArmTaskNode::execute_joint_space_trajectory(ArmSide side, const std::vector<double>& joint_angles) {
...
void ArmTaskNode::stop_arm_motion(ArmSide side) {
    publish_arm_command(side, kMotionStop, std::vector<double>(kCommandValueCount, 0.0), kTrajectoryDurationSec);
}
...
void ArmTaskNode::vision_callback(const robot_interfaces::msg::Vis& msg) {
    ...
    try {
        const auto transform = tf_buffer_->lookupTransform(kBaseFrame, camera_frame, tf2::TimePointZero, tf2::durationFromSec(0.05));
...
bool ArmTaskNode::is_supported_task_mode(int32_t task_mode) {
    return task_mode == kTaskStandby || task_mode == kTaskLeftGrasp || task_mode == kTaskRightGrasp ||
           task_mode == kTaskLeftPlace || task_mode == kTaskRightPlace || task_mode == kTaskFixedRelease ||
