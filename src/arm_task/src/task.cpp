...
void ArmTaskNode::visual_servo_publish_thread() {
    ...
    while (visual_servo_active_ && !shutdown_requested_) {
        ...
        if (!has_pose) {
    std::lock_guard<std::mutex> lock(pose_mutex_);
    if (has_visual_pose_) {                    // ← 改成 has_visual_pose_（原来是 has_object_pose_）
        pose_to_publish = latest_visual_pose_; // ← 改成 latest_visual_pose_（原来是 target_object_pose_）
        // 下面这句保持不变，让它也转到 base 系（和你 get_object_pose_in_base_frame 逻辑一致）
        if (pose_to_publish.header.frame_id != base_frame_) {
            try {
                auto transform = tf_buffer_->lookupTransform(base_frame_, pose_to_publish.header.frame_id, tf2::TimePointZero, tf2::durationFromSec(0.1));
                tf2::doTransform(pose_to_publish.pose, pose_to_publish.pose, transform);
                pose_to_publish.header.frame_id = base_frame_;
            } catch (...) { /* 失败就用 camera 系也行 */ }
        }
        has_pose = true;
    } else if (has_object_pose_) {
        pose_to_publish = target_object_pose_; // 保留原来回退，兼容你原来的行为
        has_pose = true;
    }
}
